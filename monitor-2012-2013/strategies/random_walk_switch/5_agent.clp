...
(defrule switch-to-checking
	(declare (salience 88))
	?g <- (getaway)
	(not (time_is_over))
	?cp <- (current_plan_step ?cps)
	(way_point (plan-step =(+ ?cps 1)) (plan-pos-r ?r) (plan-pos-c ?c) (plan-direction ?d))
;//~ 	(local_perc (p 5) (r ?r) (c ?c))
;//~ 	(kagent_cell (pos-r ?r) (pos-c ?c) (utility ?u&:(> ?u 10)))
	(kagent_cell (pos-r ?r) (pos-c ?c) (utility ?u&:(> ?u 10)))
;//~ 	?cps <- (current_plan_step ?)
	?fc <- (future_cell (pos-r ?) (pos-c ?))
 =>
	(retract ?g ?cp ?fc)
	(assert (future_cell (pos-r ?r) (pos-c ?c) (direction ?d)))
	(focus CHECKER)
)
